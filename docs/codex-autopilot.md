...
The user should only need to define the project goal, approve the start of work, and intervene when the automation reaches a stop condition. Codex and GPT Review should handle implementation, review guidance, repair, and merge flow as much as the repository can safely support.
...
- `priority-high`: Prefer this issue before other `codex-ready` issues.
...
7. Issues ready for implementation receive `codex-ready`.
8. Issues needing human clarification receive `needs-human` instead of `codex-ready`.

## Execution Flow

When the user says `作成開始`, `次へ`, or `次の作業に進んで`, Codex should:

1. Find open issues with `codex-ready`.
2. Prefer `priority-high` issues.
3. Otherwise choose the oldest `codex-ready` issue that is not blocked.
...
10. After merge, Codex should update `PROJECT_MEMORY.md` in a later PR if the completed work created new decisions, constraints, or next actions.
...
- No `codex-ready` issue exists.
- The next issue has `needs-human`, `blocked`, or `hold`.
...
In the same project thread, Codex should interpret those commands using this document, `PROJECT_VISION.md`, `PROJECT_MEMORY.md`, and open issues.
